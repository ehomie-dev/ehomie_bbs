...
foreach($_G['cache']['portalcategory'] as $cat){
	if(!$cat['upid'] && !$cat['closed']){
		$xiaoyu_category[]=$cat;
		$substr .=$cat['catname'];
	}
}

function category_get_wheresql($xiaaoyucatid) {
	$wheresql = '';
	if(!empty($xiaaoyucatid)) {
		$wheresql = "at.catid IN ($xiaaoyucatid ) AND at.status='0'";
	} else {
		$wheresql .= " at.status='0'";	
	}
	return $wheresql;
}
function category_get_list($cat, $wheresql, $page = 1, $xiaoyu_perpage=0) {
	global $_G;
	$cat['perpage'] = empty($cat['perpage']) ? 8 : $cat['perpage'];
	$cat['maxpages'] = empty($cat['maxpages']) ? 1000 : $cat['maxpages'];
	$perpage = $xiaoyu_perpage;
